                        ...
                        <tbody>
                            @forelse($deposits as $deposit)
                                <tr class="border-b dark:border-gray-700">
                                    ...
                                    <td class="px-4 py-2 text-sm">
                                        <a href="{{ route('deposit.detail', $deposit->id) }}" class="bg-blue-500 text-white px-4 py-2 rounded-md text-xs">Detail</a>
                                        @if($user->hasRole('admin'))
                                            @if($deposit->status == 'pending')
                                            <form action="{{ route('manual.callback', $deposit->id) }}" method="POST">
                                                @csrf
                                                <button class="bg-green-500 text-white px-4 py-2 rounded-md text-xs">Simulate Callback</button>
                                            </form>
                                            @endif
                                        @endif
                                    </td>
